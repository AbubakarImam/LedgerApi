...
v1 includes account creation, a mock funding (deposit) flow, and a mock withdrawal flow.
...
Transfer, reversal, balance enquiry, account enquiry, account creation, mock funding (deposit), mock withdrawal.
...
Both endpoints generate the account number and id server-side; status starts ACTIVE; created_at is server-assigned. Customer account numbers are 10-digit numeric, randomly generated — never sequential, since sequential numbers would let someone enumerate the customer base. No information is encoded in the digits; currency and type live only in their own columns. The first digit is never 0, so the number survives an accidental string-to-integer conversion in a downstream system at full length. System account numbers are alphanumeric: the ISO currency code followed by 9 random digits, first digit never 0 (e.g. `NGN100000021`, `USD100000043`) — system accounts are few, are read by humans in logs and ops tooling for years, and must be impossible to fat-finger as a customer number; the leading letters mean the number cannot even parse as a customer number, so the format itself acts as a validation layer. The prefix is generated from the `currency_code` column at creation and is never parsed for business logic afterward — the column remains the only authoritative source. On the rare account-number collision, the unique constraint rejects the insert and the server regenerates and retries — the same contested-creation pattern already used for the idempotency key.

`POST /deposits` (mock funding) accepts a customer account, amount, narration, and a client-supplied idempotency key (passed straight through to the transfer, so a retried deposit returns the stored result instead of funding twice). The debit side is the funding system account configured for the customer account's currency (`Funding:Accounts` in configuration, decision #25); there is no lookup among system accounts. No client timestamp — the server assigns time everywhere, including mocks (decision #10 applies with equal force). Internally it executes a normal transfer: debit the system account, credit the customer account. It reuses the transfer service wholesale — funding introduces no new mechanics; the double-entry invariant, idempotency, and locking are inherited for free.

`POST /withdrawals` (mock withdrawal) is the mirror of a deposit: it accepts a customer account, amount, narration, and a client-supplied idempotency key, and internally executes a normal transfer that debits the customer account and credits the funding account configured for the customer's currency (the same `Funding:Accounts` map deposits use, decision #25). It reuses the transfer service wholesale, so the customer's balance check (a customer account cannot go negative), idempotency, and locking are inherited; an over-withdrawal is committed as a failed transaction like any insufficient-funds transfer.
...
| 24 | System account numbers | Structured self-describing suffix (e.g. NGN-TREASURY-01) vs currency code + random digits | Currency code + 9 random digits, first digit non-zero (e.g. NGN100000021) | The create request carries no suffix field, so a descriptive suffix would have to be derived from the name, and name-derived numbers collide predictably; random digits reuse the same unique-constraint regenerate-and-retry loop as customer accounts (decision #21), and the leading letters still make the number impossible to confuse with a customer number |
| 25 | Deposit funding account | Look up any active system account in the customer's currency vs designate one explicitly (config map, or a role column on Account) | Config map of currency code to funding account number (`Funding:Accounts`); missing currency is a configuration error. The funding accounts (NGN, USD, GBP, EUR; `<CCY>100000001`) are seeded by the `SeedFundingAccounts` migration so every environment has them, and the config values must match the seeded numbers | "First active system account" is ambiguous and changes silently when another system account (fees, suspense) is added; naming the account makes the debit side deliberate and needs no schema change for a mock flow. `TransferAsync` still validates that the account exists and is Active. A role column with a unique partial index per currency is the noted path if system-account purposes multiply. Deposits also require a client-supplied idempotency key, for the same reason as transfers |
...
| 29 | Withdrawals | Out of scope for v1 vs mock withdrawal endpoint; shared vs separate settlement account | Mock withdrawal endpoint in v1, reusing the transfer service (customer to funding account), sharing the funding accounts used by deposits | Mirror of decision #15: it introduces no new mechanics, and the customer-side balance rule already prevents overdraft. A shared counterparty is enough for a mock; a real withdrawal to an external bank would need its own settlement account, noted as the future path |
